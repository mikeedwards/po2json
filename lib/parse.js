...
function cleanUpG2MEscaping(result, stringify, pretty) {
  if (stringify) {
    return JSON.stringify(result, null, pretty ? '   ' : null).replace('\\\\{', '{').replace('\\\\}', '}');
  } else {
...
module.exports = function (buffer, options) {
  ...
  if (options.format === 'mf') {
    ...
    return cleanUpG2MEscaping(mfTranslations, options.stringify, options.pretty)
  }
